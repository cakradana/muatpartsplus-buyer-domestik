...
// Backend to Frontend status mapping for Transporter module
// Maps backend statuses that are not directly supported in Transporter to appropriate frontend statuses
// This allows the frontend to handle backend statuses that don't have direct equivalents in the Transporter workflow
// For example: WAITING_CONFIRMATION_CHANGES (from backend) -> NEED_CHANGE_RESPONSE (frontend handling)
export const mapBackendStatusToFrontend = (backendStatus) => {
  const statusMapping = {
    WAITING_CONFIRMATION_CHANGES: ORDER_STATUS.NEED_CHANGE_RESPONSE,
    // Add other mappings as needed
    // BACKEND_STATUS: ORDER_STATUS.FRONTEND_STATUS,
  };
